...
let fsx = Path.Combine(__SOURCE_DIRECTORY__, __SOURCE_FILE__)
let template = Path.Combine(__SOURCE_DIRECTORY__, "template.html")
Literate.ProcessScriptFile(fsx, template)
let fsx' = Path.Combine(__SOURCE_DIRECTORY__, "accumulate.fsx")
Literate.ProcessScriptFile(fsx', template)